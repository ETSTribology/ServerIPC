...
class TestCustomLogHandlers:
    @pytest.mark.skipif(
        "SKIP_EXTERNAL_TESTS" in os.environ, reason="Skipping tests requiring external services"
    )
    ...
    def test_minio_handler_initialization(self):
        ...
        handler = MinIOHandler(
            minio_client=mock_minio_client, bucket_name="test-logs", object_name="test-log.json"
        )
