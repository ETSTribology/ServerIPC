...
class TestTransformationUtils:
    ...
    def test_apply_rotation(self):
        ...
        expected_vertices = np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]])
    ...
    def test_apply_transformations_invalid_input(self):
        ...
        with pytest.raises(ValueError):
            apply_transformations(
                vertices, scale=[1, 2], rotation=[0, 0, 0, 1], translation=[0, 0, 0]
            )

        with pytest.raises(ValueError):
            apply_transformations(vertices, scale=[1, 1, 1], rotation=[0, 0], translation=[0, 0, 0])

        with pytest.raises(ValueError):
            apply_transformations(
                vertices, scale=[1, 1, 1], rotation=[0, 0, 0, 1], translation=[0, 0]
            )
