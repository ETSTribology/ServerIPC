...
from simulation.core.utils.io.io import load_individual_meshes, load_mesh


class TestIOMeshLoading:
    @pytest.fixture(scope="module")
    def config_manager(self):
        """Fixture to initialize ConfigManager."""
        ConfigManager.reset()
        config_manager = ConfigManager.get_instance()
        config_manager.initialize(
            config_name="config.yaml", config_path="./configs"
    ...
    def test_load_mesh_no_tetra_cells(self):
        ...
        with tempfile.NamedTemporaryFile(mode="wb", suffix=".msh", delete=False) as temp_file:
            try:
                ...
                points = np.array(
                    [[0, 0, 0], [1, 0, 0], [0, 1, 0]],  # Vertex 0  # Vertex 1  # Vertex 2
                    dtype=np.float64,
                )
    ...
    def test_load_individual_meshes(self, config_manager):
        ...
        with tempfile.NamedTemporaryFile(mode="wb", suffix=".msh", delete=False) as temp_file:
            try:
                ...
                # Load meshes
                meshes, mesh_configs = load_individual_meshes(inputs, config_manager)
