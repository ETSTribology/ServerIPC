...
class SimulationState:
    ...
    def add_attribute(self, key: str, value: Any, aliases: List[str] = None) -> None:
        ...
        # Then validate all aliases if provided
        if aliases:
        ...
        # After validation, perform the updates
    ...
    def save_to_db(self, db: 'DatabaseBase', table: str = 'simulation_states', state_id: str = None) -> str:
        """Saves the current simulation state to the database.
        ...
        table : str, optional
            The table name to save the state to, by default 'simulation_states'
        state_id : str, optional
            The ID to use for the state record. If None, a new record will be created.
        ...
        if state_id is None:
            # Create new record
            result = db.create(table, state_data)
            return result.get('id')
        else:
            # Update existing record
            result = db.update(table, state_id, state_data)
            return state_id

    def load_from_db(self, db: 'DatabaseBase', table: str = 'simulation_states', state_id: str = None) -> None:
        """Loads a simulation state from the database.

        Parameters
        ----------
        db : DatabaseBase
            The database instance to load from.
        table : str, optional
            The table name to load the state from, by default 'simulation_states'
        state_id : str, optional
            The ID of the state record to load. If None, loads the most recent state.

        Raises
        ------
        ValueError
            If no state record is found.
        """
        if state_id is None:
            # Get most recent state
            results = db.query(f"SELECT * FROM {table} ORDER BY created_at DESC LIMIT 1")
            if not results:
                raise ValueError("No simulation states found in database")
            state_data = results[0]
        else:
            # Get specific state
            state_data = db.get(table, state_id)
            if not state_data:
                raise ValueError(f"No simulation state found with ID: {state_id}")

        self.from_dict(state_data['state'])
        self.aliases = state_data.get('aliases', {})
