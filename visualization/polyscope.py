...
class Polyscope:
    def __init__(
        self, config: VisualizationConfigManager, storage: Storage, backend: Backend, board: Board
    ):
        self.config = config
        ...
        self.performance_tracking = (
            config.get().get("visualization", {}).get("performanceTracking", False)
        )
        ...
        self.screenshot_enabled = (
            config.get().get("extensions", {}).get("screenshot", {}).get("enabled", False)
        )
        self.mesh_save_enabled = (
            config.get().get("extensions", {}).get("mesh", {}).get("enabled", False)
        )
        self.board_url = (
            f"http://{config.get().get('extensions', {}).get('board', {}).get('host', 'localhost')}:"
    ...
    def menu(self):
        ...
        changed, self.performance_tracking = psim.Checkbox(
            "Performance Tracking", self.performance_tracking
        )
        if changed:
        ...
        if psim.TreeNode("Simulation Controls"):
            ...
            # Dropdown for steps
            if self.steps:
                changed, self.selected_step = psim.Combo(
                    "Steps", self.selected_step or "", self.steps
                )
                if changed:
        ...
        if psim.TreeNode("Extensions"):
            ...
            if psim.TreeNode("Screenshot"):
                changed, self.screenshot_enabled = psim.Checkbox(
                    "Enable Screenshots", self.screenshot_enabled
                )
                if changed:
            ...
            if psim.TreeNode("Mesh Saving"):
                ...
                mesh_dir = (
                    self.config.get().get("extensions", {}).get("mesh", {}).get("directory", "")
                )
            ...
            if psim.TreeNode("Board"):
                ...
                board_dir = (
                    self.config.get().get("extensions", {}).get("board", {}).get("directory", "")
                )
